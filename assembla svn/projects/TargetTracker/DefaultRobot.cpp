...
public:
	...
	DefaultRobot(void) {
		...
		finder = new Target10Finder(camera);
	...
	void OperatorControl(void) {
		...
		while (IsOperatorControl()) {
			GetWatchdog().Feed();
			//finder->update();
			CameraObject *target = finder->getObject();
			printf("size=%f x=%f y=%f\n dist: %f",target->size,target->xAngle,target->yAngle,target->getZ(50));
			delete target;
			
			if((t2->Get() > 0.2) && isKicked ) {
				isKicked = false;
				printf("Unkick %f\n", t2->Get());
				t->Reset();
			}
