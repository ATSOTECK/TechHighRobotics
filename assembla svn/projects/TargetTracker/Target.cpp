...
vector<Target> Target::FindCircularTargets(ColorImage *image)
{
	...
	for (unsigned i = 0; i < results->size(); i++)
	{
		...
		target.m_score = (e.majorRadius * e.minorRadius)
							/ (1001 - e.score)
							/ (height * width) * 100;
		target.m_majorRadius = e.majorRadius; //EDIT: measurements used to be height=0 now thy are pixel=0
		target.m_minorRadius = e.minorRadius;
		//always divide by height so that x and y are same units
		target.m_xPos = e.position.x;
        target.m_yPos = e.position.y;
